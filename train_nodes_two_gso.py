...
from models.gcn_conv_two_gso import GCN_Two_GSO
...
parser.add_argument('--gso_1', type=str, default='NORMADJ',  choices=['ADJ', 'UNORMLAPLACIAN','SINGLESSLAPLACIAN','RWLAPLACIAN','SYMLAPLACIAN','NORMADJ', 'MEANAGG', 'Test'])
parser.add_argument('--gso_2', type=str, default='NORMADJ',  choices=['ADJ', 'UNORMLAPLACIAN','SINGLESSLAPLACIAN','RWLAPLACIAN','SYMLAPLACIAN','NORMADJ', 'MEANAGG', 'Test'])
...
parser.add_argument('--use_wandb', type= bool,default = True , choices=[True, False])
...
for t_ in range(10) :
    ...
    # Model and optimizer
    model = GCN_Two_GSO(input_dim=features.shape[1],
                hidden_dim=args.hidden,
                output_dim=labels.max().item() + 1,
                num_layers=args.num_layers,
                dropout=args.dropout, gso_1 = args.gso_1, gso_2 = args.gso_2).to(device)
    
    # Freezing the weights of the PGSO
    model.m1_1.requires_grad = False
    model.m2_1.requires_grad = False
    model.m3_1.requires_grad = False
    model.e1_1.requires_grad = False
    model.e2_1.requires_grad = False
    model.e3_1.requires_grad = False
    model.a_1.requires_grad = False


        # Freezing the weights of the PGSO
    model.m1_2.requires_grad = False
    model.m2_2.requires_grad = False
    model.m3_2.requires_grad = False
    model.e1_2.requires_grad = False
    model.e2_2.requires_grad = False
    model.e3_2.requires_grad = False
    model.a_2.requires_grad = False
